...
export function EmailRecipients({ completedCall, lead, transcript = [], startedAt, durationSeconds }) {
  const initialRecipients = useRef(readStored(STORAGE_KEYS.recipients, ["jaferm@aqionlabs.com"]) || ["jaferm@aqionlabs.com"]).current;
  ...
  const latestCall = useMemo(() => {
    if (completedCall?.lead || completedCall?.transcript?.length) return { ...completedCall, lead: completedCall.lead || lead };
  ...
  async function handleSend() {
    ...
    const subject = "AqionVOX — call summary and captured lead";
    ...
    try {
      ...
      if (result.configured === false) {
        ...
        setFeedback({ tone: "ok", text: "Email draft opened. Press Send in your mail app, or configure SMTP for automatic delivery." });
        return;
      }
